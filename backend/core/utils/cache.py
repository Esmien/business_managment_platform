...
def rbac_cache(ttl: int = 3600) -> Callable:
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(self: Any, role_id: int, business_element_name: str) -> AccessRuleDTO | None:
            ...
            if cached_data:
                logger.debug(f"CACHED: Правило {cached_data} получено для роли {role_id} к {business_element_name}.")
            ...
            # Кэшируем результат
            if result:
                await self.redis.setex(name=cache_key, time=ttl, value=result.model_dump_json())
