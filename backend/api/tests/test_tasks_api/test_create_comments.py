...
async def test_add_comment_success(client, task_in_json):
    ...
    response = await client.post(f"/api/v1/tasks/{task_id}/comments", json=comment_data)
...
async def test_add_comment_task_not_found(client):
    ...
    response = await client.post("/api/v1/tasks/99999/comments", json=comment_data)
