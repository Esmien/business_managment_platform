...
from backend.task.schemas import TaskRead, TaskCreate, TaskUpdate, CommentCreate, TaskChangeStatus
from backend.exceptions import TaskDoesNotExistsError, AccessDeniedError, TeamDoesNotExistsError, UserDoesNotExistsError
...
async def test_create_task(task_service, mock_uow, mock_user_author, sample_task):
    ...
    mock_uow.tasks.create_task.assert_called_once_with(task_in=task_in, author_id=mock_user_author.id)
    mock_uow.commit.assert_called_once()


async def test_update_task_access_denied(task_service, mock_uow, mock_user_stranger, sample_task):
    # Пытаемся обновить чужую задачу без прав менеджера/админа
    mock_uow.tasks.get_task_by_id.return_value = sample_task
    update_data = TaskUpdate(title="Взлом")

    with pytest.raises(AccessDeniedError):
        await task_service.update_task(task_id=1, update_data=update_data, user=mock_user_stranger)


async def test_update_task_success(task_service, mock_uow, mock_user_author, sample_task):
    ...
    result = await task_service.update_task(task_id=1, update_data=update_data, user=mock_user_author)

    assert result.title == "Обновлено"
    mock_uow.commit.assert_called_once()


async def test_delete_task_success(task_service, mock_uow, mock_user_author, sample_task):
    mock_uow.tasks.get_task_by_id.return_value = sample_task
...
async def test_add_comment_success(task_service, mock_uow, mock_user_author, sample_task, sample_comment):
    ...
    result = await task_service.add_comment(task_id=1, user=mock_user_author, comment_in=comment_in)

    assert result.text == "Тестовый комментарий"
    mock_uow.tasks.create_comment.assert_called_once_with(
        task_id=1,
        author_id=mock_user_author.id,
        text="Тестовый комментарий"
    )
    mock_uow.commit.assert_called_once()


async def test_add_comment_access_denied(task_service, mock_uow, mock_user_stranger, sample_task):
    # Имитируем найденную задачу
    mock_uow.tasks.get_task_by_id.return_value = sample_task
    ...
    with pytest.raises(AccessDeniedError):
        await task_service.add_comment(task_id=1, user=mock_user_stranger, comment_in=comment_in)
...
async def test_add_comment_task_not_found(task_service, mock_uow, mock_user_author):
    ...
    with pytest.raises(TaskDoesNotExistsError):
        await task_service.add_comment(task_id=999, user=mock_user_author, comment_in=comment_in)
...
async def test_get_filtered_tasks_team_scope(task_service, mock_uow, mock_user_author):
    ...
    result = await task_service.get_filtered_tasks(user=mock_user_author, scope="team", task_status=None)
    assert result == []
    mock_uow.tasks.get_tasks_with_filters.assert_called_once_with(user_id=None, team_id=1, task_status=None)


async def test_get_filtered_tasks_team_scope_no_team(task_service, mock_uow, mock_user_author):
    mock_user_author.team_id = None
    with pytest.raises(TeamDoesNotExistsError):
        await task_service.get_filtered_tasks(user=mock_user_author, scope="team", task_status=None)


async def test_get_filtered_tasks_all_scope_admin(task_service, mock_uow, mock_user_author):
    mock_user_author.role.name = RoleName.ADMIN
    mock_uow.tasks.get_tasks_with_filters.return_value = []

    result = await task_service.get_filtered_tasks(user=mock_user_author, scope="all", task_status=None)
    assert result == []


async def test_update_task_empty_dict(task_service, mock_uow, mock_user_author, sample_task):
    mock_uow.tasks.get_task_by_id.return_value = sample_task
    # Пустой update_data
...
async def test_update_task_invalid_executor(task_service, mock_uow, mock_user_author, sample_task):
    mock_uow.tasks.get_task_by_id.return_value = sample_task
    ...
    with pytest.raises(UserDoesNotExistsError):
        await task_service.update_task(task_id=1, update_data=update_data, user=mock_user_author)


async def test_update_task_repo_fails(task_service, mock_uow, mock_user_author, sample_task):
    mock_uow.tasks.get_task_by_id.return_value = sample_task
...
async def test_change_status_success(task_service, mock_uow, mock_user_author, sample_task):
    mock_uow.tasks.get_task_by_id.return_value = sample_task
    mock_uow.tasks.update_task.return_value = sample_task
    new_status = TaskChangeStatus(status=TaskStatus.DONE)

    result = await task_service.change_status(task_id=1, new_status=new_status, user=mock_user_author)
    assert result == sample_task


async def test_change_status_access_denied(task_service, mock_uow, mock_user_stranger, sample_task):
    mock_uow.tasks.get_task_by_id.return_value = sample_task
    new_status = TaskChangeStatus(status=TaskStatus.DONE)

    with pytest.raises(AccessDeniedError):
        await task_service.change_status(task_id=1, new_status=new_status, user=mock_user_stranger)
...
async def test_change_status_task_not_found(task_service, mock_uow, mock_user_author):
    ...
    with pytest.raises(TaskDoesNotExistsError):
        await task_service.change_status(task_id=1, new_status=new_status, user=mock_user_author)
...
async def test_change_status_repo_fails(task_service, mock_uow, mock_user_author, sample_task):
...
async def test_permissions_as_executor_and_manager(
    ...
    await task_service.change_status(
        task_id=1, new_status=TaskChangeStatus(status=TaskStatus.DONE), user=mock_user_stranger
    )
    # Оставляем коммент как исполнитель
    await task_service.add_comment(
        task_id=1, user=mock_user_stranger, comment_in=CommentCreate(text="Коммент исполнителя")
    )
    ...
    await task_service.add_comment(
        task_id=1, user=mock_user_stranger, comment_in=CommentCreate(text="Коммент менеджера")
    )


async def test_update_task_not_found_initially(task_service, mock_uow, mock_user_author):
    """
